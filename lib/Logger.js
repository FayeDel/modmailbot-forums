...
module.exports = class Logger {
	...
	async created(guild, user) {
		...
		await thread.send({ content: `<@&${role}> **${user}** [${user.id}] opened this ticket on ${timestamp()}`, allowedMentions: { roles: [role] } });
	...
	async closed(guild, user, moderator) {
		...
		await channel.send(`${success} ticket for **${user}** [${user.id}] closed by ${moderator} [${moderator.id}] on ${timestamp()}`);
		await user.send([
